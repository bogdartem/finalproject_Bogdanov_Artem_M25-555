...
class UserManager:
    ...
    def register_user(self, username: str, password: str) -> User:
        ...
        user = User(
            user_id, username, hashed_password, salt, registration_date)
    ...
    def _create_user_portfolio(self, user_id: int):
        ...
        if not any(
            portfolio["user_id"] == user_id for portfolio in portfolios_data
        ):
...
class PortfolioManager:
    def __init__(
        self, data_manager: DataManager, rate_service: ExchangeRateService
    ):
        self.data_manager = data_manager
    ...
    def sell_currency(
        ...
        if not wallet:
            raise ValueError(
                f"You don't have wallet for currency '{currency_code}'")
